...
vector<vector<Edge>> addFeasibleEdges(const VRP &vrp, const vector<vector<Edge>> &graph,int cnt) {
  ...
  for(auto nodes: graph){
    ...
    for(int i=0;i<cnt;i++){
      ...
      if(existingNodes.find(v)!=existingNodes.end()){
        continue; // already exists
      }
      // check if edge u-v is feasible
      if(vrp.node[u].earlyTime+vrp.node[u].serviceTime+vrp.get_dist(u,v)<=vrp.node[v].latestTime){
        // feasible
        feasibleGraph[u].push_back(Edge(v, vrp.get_dist(u,v)));
        feasibleGraph[v].push_back(Edge(u, vrp.get_dist(u,v)));
        existingNodes.insert(v);
        cout << "Feasible Edge added: " << u << " -- " << v << '\n';
      }
...
int main(int argc, char *argv[]) {
  ...
  mstG= addFeasibleEdges(vrp, mstG,2);
  //printAdjList(mstG);
