...
// Adding all the edge from where the next node is fessible.
struct EdgeNode{
  node_t to;
  tw_t  process_time;
};

vector<vector<Edge>> addFeasibleEdges(const VRP &vrp, const vector<vector<Edge>> &graph,int cnt) {
  size_t N = graph.size();
  vector<vector<Edge>> feasibleGraph(N);
  int u=0;
  for(auto nodes: graph){
    set<node_t> existingNodes;
    for(auto v: nodes){
      feasibleGraph[u].push_back(v); // add all edges from node "v.to"
      existingNodes.insert(v.to);
    }
    
      //get the v between the 0 and N-1 excluding u and existing nodes
      vector<EdgeNode> candidateEdges;
      for(node_t v=0;v<N;v++){
        if(v==u) continue;
        if(existingNodes.find(v)!=existingNodes.end()) continue;
        //check if adding edge u-v is feasible
        tw_t process_time= vrp.node[u].serviceTime + vrp.get_dist(u,v);
        process_time=max(process_time,vrp.node[v].earlyTime);
        if(process_time <= vrp.node[v].latestTime){
          candidateEdges.push_back({v,process_time});
        }
      }
      sort(candidateEdges.begin(),candidateEdges.end(),[](const EdgeNode &a, const EdgeNode &b){
        return a.process_time < b.process_time;
      });
      //add top cnt edges
      for(int i=0;i<min(cnt,(int)candidateEdges.size());i++){
        node_t v=candidateEdges[i].to;
        weight_t w= vrp.get_dist(u,v);
        feasibleGraph[u].push_back(Edge(v,w));
        feasibleGraph[v].push_back(Edge(u,w));
      }
    u++;
  }

  return feasibleGraph;
}
...
int main(int argc, char *argv[]) {
  ...
  printAdjList(mstG);
  // mstG now has the adjacency list of MST
  mstG= addFeasibleEdges(vrp, mstG,1);
  printAdjList(mstG);
